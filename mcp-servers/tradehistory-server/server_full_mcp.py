...
health_app = FastAPI(title="TradeHistory MCP Server Health")
...
mcp = FastMCP("TradeHistory Server")

@mcp.list_resources()
async def list_resources() -> list[Resource]:
    """List available trade history resources."""
    return [
        Resource(
            uri="tradehistory://trades",
            name="Trade History",
            description="Get historical trade information",
            mimeType="application/json"
        ),
        Resource(
            uri="tradehistory://summary",
            name="Trade Summary",
            description="Get trade summary and statistics",
            mimeType="application/json"
        )
    ]
...
async def read_resource(uri: str) -> str:
    """Read a specific trade history resource."""
    if uri == "tradehistory://trades":
        return "Use get_trade_history tool to fetch historical trades"
    elif uri == "tradehistory://summary":
        return "Use get_trade_history tool to fetch trade statistics"
    else:
        raise ValueError(f"Unknown resource: {uri}")

@mcp.list_tools()
async def list_tools() -> list[Tool]:
    """List available trade history tools."""
    return [
        Tool(
...
async def call_tool(name: str, arguments: dict) -> Sequence[dict]:
    """Execute a trade history tool."""
    logger.info(f"Calling tool {name} with arguments {arguments}")
    
    if name == "get_trade_history":
        result = await _get_trade_history(arguments["owner"])
    else:
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting TradeHistory MCP server with stdio transport")
